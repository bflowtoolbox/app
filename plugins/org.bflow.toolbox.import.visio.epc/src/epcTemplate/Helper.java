...
public class Helper{
	...
	protected static EPackage standardDocumentStencilPackage2003;
	protected static EPackage standardDocumentStencilPackage2007;
	...
	public static void init(String metamodelPath, String visioVersion) {
		
		URL url = FileLocator.find(Activator.getDefault().getBundle(), new Path(metamodelPath),
				null);
		URL metaModelURL;
		try {
			...
			if (visioVersion.equals("2007")){
				standardDocumentStencilPackage2007 = ((EPackage) res.getContents().get(
					0)).getESubpackages().get(0);
			}
			
			if (visioVersion.equals("2003")){
				standardDocumentStencilPackage2003 = ((EPackage) res.getContents().get(
					0)).getESubpackages().get(0);
			}
	...
	public static List<EObject> selectCorrectShapes(List<EObject> allShapes,
		...
		if (!allShapes.isEmpty()) {
			...
			for (int i = 0; i < allShapes.size(); i++) {
				if (isInOurStencil(((EObject) allShapes.get(i)), standardDocumentStencilPackage2007) || isInOurStencil(((EObject) allShapes.get(i)), standardDocumentStencilPackage2003)) {
	...
	private static boolean isInOurStencil(EObject shapeEObject, EPackage standardDocumentStencilPackage) {
