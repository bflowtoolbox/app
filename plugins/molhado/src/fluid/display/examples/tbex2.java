...
public class tbex2 extends JFrame implements TextClickedListener, TextResizedListener {
  ...
  private void styleText() {
	...
	if( lines[i].charAt( j ) == ' ' ) {
	  if( count == 0 )
	    tb.applyStyle( plain, i, start, i, j-1 );
	  else if( count == 1 )
	    tb.applyStyle( bold, i, start, i, j-1 );
	  else
	    tb.applyStyle( italic, i, start, i, j-1 );
	  count = (count+1) % 3;
      ...
      if( count == 0 )
	tb.applyStyle( plain, i, start, i, lines[i].length()-1 );
      else if( count == 1 )
	tb.applyStyle( bold, i, start, i, lines[i].length()-1 );
      else
	tb.applyStyle( italic, i, start, i, lines[i].length()-1 );
      count = (count+1)%3;
    }
    tb.repaint();
  }
  ...
  private String text;
  private TextBean tb;
  private TextStyle ts, bold, italic, plain;

  public tbex2( String txt ) {
    text = txt;
    lines = null;

    ts = new TextStyle( Color.red, Color.black );
    bold = new TextStyle( true, true, false, false, false, null, false, null );
    italic = new TextStyle( false, false, true, true, false, null,
			    false, null );
    plain = new TextStyle( true, false, true, false, false, null, false, null);
