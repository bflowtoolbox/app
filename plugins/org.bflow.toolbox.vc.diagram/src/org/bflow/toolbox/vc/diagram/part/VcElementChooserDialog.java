 ...
 * @generated
 */
public class VcElementChooserDialog extends Dialog {
	...
	protected Control createDialogArea(Composite parent) {
		...
		getShell().setText(
				Messages.VcElementChooserDialog_SelectModelElementTitle);
	...
	private boolean isValidModelFile(IFile file) {
		...
		return "vc".equals(fileExtension)||"epc".equals(fileExtension); //$NON-NLS-1$
	}
			...
			if (event.getSelection() instanceof IStructuredSelection) {
				...
				if (selection.size() == 1) {
					...
					if (selectedElement instanceof EObject) {
						...
						setOkButtonEnabled(ViewService.getInstance().provides(
								Node.class,
								new EObjectAdapter(selectedModelElement),
								myView, null, ViewUtil.APPEND, true,
