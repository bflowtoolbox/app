...
import oepc.impl.OepcFactoryImpl;
...
/**
 * Provides an filter for oepc model changes.
 * Also implements the filtering manual to test new values.
 * @author Jörg
 *
 */
class OepcNotificationMatcher {

	
	/**
	 * Create me.
	 */
	OepcNotificationMatcher(){
	...
	public boolean matchesModification(Notification n){
		...
		OepcPackage oepc = OepcFactoryImpl.eINSTANCE.getOepcPackage();
		EAttribute beName = oepc.getBusinessObjectElement_Name();
	...
	/**
	 * The default filter for oepc model changes.
	...
	public static NotificationFilter buildFilter(){
		...
		NotificationFilter f4 = NotificationFilter.createFeatureFilter(beName);
		NotificationFilter f5 = NotificationFilter.createFeatureFilter(nodes);
		
		filter = f1.or(f2).or(f3).or(f4).or(f5);
		return filter;
	}
}
