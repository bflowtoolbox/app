 ...
 * @since 22.11.09
 * @version 12.07.2015 AST - Using getAdapter() instead of hard cast
 *
 */
public class ElementGeneratorAction implements IObjectActionDelegate {
	...
	public void setActivePart(IAction action, IWorkbenchPart targetPart) {
		...
		editor = (EpcDiagramEditor) activePart.getSite().getWorkbenchWindow().getActivePage().getActiveEditor().getAdapter(EpcDiagramEditor.class);
	}

	@Override
	public void run(IAction action) {								
		WizardDialog wd = new WizardDialog(activePart.getSite().getShell(), new ModelWizard(editor));
