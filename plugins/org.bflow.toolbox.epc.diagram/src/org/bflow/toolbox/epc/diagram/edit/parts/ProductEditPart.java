...
import org.bflow.toolbox.epc.diagram.edit.policies.ProductItemSemanticEditPolicy;
import org.bflow.toolbox.epc.diagram.part.EpcVisualIDRegistry;
import org.bflow.toolbox.epc.extensions.edit.parts.ProductBorder;
...
public class ProductEditPart extends BflowNodeEditPart {
	...
	public class ProductFigure extends RectangleFigure {
		...
		private IFigure productPolygonFigure;
		...
		private void createContents() {
			...
			class ProductPolygonFigure0Class extends Shape {
				/**
				 * @generated
				 */
			...
			if(diagramEditPart != null){
				productPolygonFigure0.setBackgroundColor(
						diagramEditPart.getColorSchema().getBackground(
								ProductEditPart.class));
				productPolygonFigure0.setForegroundColor(
						diagramEditPart.getColorSchema().getForeground(
								ProductEditPart.class));
			}
			else{
				productPolygonFigure0.setBackgroundColor(ColorConstants.white);
				productPolygonFigure0.setForegroundColor(
						ColorConstants.black);
			}
			
			productPolygonFigure0.setBorder(new CompoundBorder(
					new MarginBorder(getMapMode().DPtoLP(0), getMapMode()
							.DPtoLP(0), getMapMode().DPtoLP(10), getMapMode()
							.DPtoLP(0)), createBorder0()));

			this.add(productPolygonFigure0);
			productPolygonFigure = productPolygonFigure0;
			layoutLabelContainer(productPolygonFigure0, diff);
		...
		private Border createBorder0() {
			ProductBorder result = new ProductBorder();
		...
		public IFigure getProductPolygonFigure() {
			return productPolygonFigure;
		}

	}

	public IFigure getPrimaryFigure() {
		return getPrimaryShape().getProductPolygonFigure();
	}

	public WrappingLabel[] getLabels() {
		return new WrappingLabel[] { ((ProductFigure) primaryShape)
				.getFigureProductNameFigure() };
	}
}
