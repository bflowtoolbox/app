 ...
 * @author arupghose
 */
public class ApplyConflictHandler extends ReasonerHandler{
	
	ModelImpl model;
	CommandStack cs;

	public ApplyConflictHandler() {
		super();
	}
			...
			if (selection instanceof IStructuredSelection) {
				...
				for (int i = 0; i < selectionSize; i++) {
					...
					try {
						...
						SetQualitativeEvaluationLabelCommand setLabel = new SetQualitativeEvaluationLabelCommand(partIntention, EvaluationLabel.CONFLICT);
						cs.execute(setLabel);
