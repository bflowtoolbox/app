 ...
 * @author arupghose
 */
public class ApplyUnknownHandler extends ReasonerHandler{
	
	ModelImpl model;
	CommandStack cs;

	public ApplyUnknownHandler() {
		super();
	}
			...
			if (selection instanceof IStructuredSelection) {
				...
				for (int i = 0; i < selectionSize; i++) {
					...
					try {
						...
						SetQualitativeEvaluationLabelCommand setLabel = new SetQualitativeEvaluationLabelCommand(partIntention, EvaluationLabel.UNKNOWN);
						cs.execute(setLabel);
